...
class DBAL extends PDO {
	...
	public function getRows( string $sql, array $bindings = [], int $fetch_type = PDO::FETCH_ASSOC ): array {
	...
	public function getRow( string $sql, array $bindings = [], int $fetch_type = PDO::FETCH_ASSOC ): array {
		$sth = $this->execute( $sql, $bindings );
	...
	public function getColumn( string $sql, array $bindings = [], int $column_number = 0 ): array {
		$sth    = $this->execute( $sql, $bindings );
	...
	public function getCell( string $sql, array $bindings = [], int $column_number = 0 ): ?string {
		$sth = $this->execute( $sql, $bindings );
	...
	public function generateRows( string $sql, array $bindings = [], int $fetch_type = PDO::FETCH_ASSOC ): Iterator {
		$sth = $this->execute( $sql, $bindings );
	...
	public function generateColumn( string $sql, array $bindings = [], int $column_number = 0 ): Iterator {
		$sth = $this->execute( $sql, $bindings );
	...
	public function update( string $table, array $data, array $conditions = [], array $bindings = [] ): int {
		$columns = [];
		foreach ( $data as $k => $v ) {
			$columns[] = "{$k} = :{$k}";
	...
	public function delete( string $table, array $conditions = [], array $bindings = [] ): int {
	...
	public static function bindValues( PDOStatement $sth, array $bindings = [] ): void {
